...
parser.add_argument('--dataset', type=str, default= DATANAME, help="Dataset to use.")
parser.add_argument('--dataset_test', type=str, default= DATANAME_TEST, help="Dataset to use.")
parser.add_argument('--model', type=str, default= model_names,
                    ...
                    "mou (1D RNN),"
                    "nonlocalnetwork")
parser.add_argument('--folder', type=str, help="Folder where to store the datasets (defaults to the current working directory).", default= DATAROOT)
parser.add_argument('--cuda', type=int, default=0, help="Specify CUDA device (defaults to 0, which learns on CPU)")
...
group_dataset.add_argument('--training_sample', type=float, default=0.2, help="Percentage of samples to use for training (default: 10%)")
group_dataset.add_argument('--sampling_mode', type=str, help="Sampling mode (random sampling or disjoint, default: random)", default='nonlocalnetwork')
group_dataset.add_argument('--train_set', type=str, default='sh/sh1', help="Path to the train ground truth (optional, this supersedes the --sampling_mode option)")
group_dataset.add_argument('--test_set', type=str, default='sh/sh2', help="Path to the test set (optional, by default the test_set is the entire ground truth minus the training)")
# Training options
group_train = parser.add_argument_group('Training')
group_train.add_argument('--epoch', type=int, default = 100, help="Training epochs (optional, if absent will be set by the model)")
group_train.add_argument('--patch_size', default = 5, type=int, help="Size of the spatial neighbourhood (optional, if absent will be set by the model)")
...
parser.add_argument('--download', type=str, default=None, nargs='+', choices=DATANAME, help="Download the specified datasets and quits.")
parser.add_argument('--test_only', default=False, help="Skip training and test only.")
parser.add_argument('--cal_only', default=False, help="Skip training and testing and calculate only.")

args = parser.parse_args()

CUDA_DEVICE = get_device(0 if torch.cuda.is_available() else -1)


TEST_ONLY = (args.test_only or args.cal_only)
...
DATASET_TEST = args.dataset_test # Dataset_test name
...
_img, _gt, _LABEL_VALUES, _IGNORED_LABELS, _RGB_BANDS, _palette = get_dataset(DATASET_TEST, FOLDER)
# Number of classes
N_CLASSES = len(LABEL_VALUES) -  len(IGNORED_LABELS)
# N_CLASSES = len(LABEL_VALUES)
# Number of bands (last dimension of the image tensor)
N_BANDS = img.shape[-1]
N_BANDS_T = _img.shape[-1]
...
hyperparams.update({'patch_size' : PATCH_SIZE, 'n_classes' : N_CLASSES, 'n_bands' : N_BANDS, 'ignored_labels' : IGNORED_LABELS, 'device' : CUDA_DEVICE})
hyperparams = dict((k, v) for k, v in hyperparams.items() if v is not None)
#print('\n\n\n', hyperparams, '\n\n\n')

# Show the image and the ground truth
display_dataset(_img, gt, RGB_BANDS, LABEL_VALUES, palette, viz)
_color_gt = convert_to_color(_gt)
...
if DATAVIZ:
    ...
    mean_spectrums = explore_spectrums(_img, gt, LABEL_VALUES, viz, ignored_labels=IGNORED_LABELS)
    plot_spectrums(mean_spectrums, viz, title='Mean spectrum/class')
...
if TRAIN_GT is not None and TEST_GT is not None:
    train_gt = open_file(TRAIN_GT+'_gt.mat')
    train_gt = train_gt['sh1_gt']
    test_gt = open_file(TEST_GT+'_gt.mat')
    test_gt = test_gt['sh2_gt']
...
#display_predictions(convert_to_color(train_gt[TRAIN_GT+'_gt']), viz, caption="Train ground truth")
#display_predictions(convert_to_color(test_gt[TRAIN_GT+'_gt']), viz, caption="Test ground truth")
...
def neighbor_add( row, col, new_train_gt):
    ...
    for i in range(-t, t + 1):
        for j in range(-t, t + 1):
            if i + row < 0 or i + row >= da_ or j + col < 0 or j + col >= da_:
                continue
            else:
...
"""change"""
test_dataset = HyperX(_img, test_gt, **hyperparams)
...
for time_index in range(0, 1):
    ...
    # train ##################################################################################
      total_eopchs = EPOCH
      for epoch_index in range(total_eopchs):
        st = time.time()
        ...
        for train_batch_index, (img_batch, label_batch, indices) in enumerate(train_loader):
            ...
            if loss2 <= loss_low2:
                ...
                print('saved loss2 =', loss2)
                loss_low2 = loss2
        """change"""
        try:
            mean_acc_train = total_acc_train * 1.0 / total_sample_train#.item() * 1.0 / total_sample_train
        except:
            mean_acc_train = 0
        try:
        ...
        print('[Train] epoch[%d/%d] acc:%.4f%% loss:%.4f loss1:%.4f loss_un:%.4f loss2:%.4f\n'
              % (epoch_index, total_eopchs, mean_acc_train * 100, mean_loss_train, loss1, loss_un, loss2))#.item(), loss1, loss_un, loss2))
    ...
    if not CAL_ONLY:
      ...
      sio.savemat(os.path.join(result_path, 'total_labels.mat'), {'total_labels': total_labels})
    ...
    # aa oa kappa ################################################################################
    if CAL_ONLY:
        total_predicts = scio.loadmat(os.path.join(result_path, 'pre_label.mat'))['pre_label'][0]
        total_test = scio.loadmat(os.path.join(result_path, 'test_label.mat'))['test_label'][0]
        total_indices = scio.loadmat(os.path.join(result_path, 'total_indices.mat'))['total_indices']
        #total_labels = scio.loadmat(os.path.join(result_path, 'total_labels.mat'))['total_labels']
        total_labels = ['Water', 'Ground/Building', 'Plant']
