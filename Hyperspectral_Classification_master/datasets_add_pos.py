...
from Hyperspectral_Classification_master.utils import open_file
...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    if dataset_name == 'PaviaC':
        ...
        # Custom dataset
        img, gt, rgb_bands, ignored_labels, label_values = CUSTOM_DATASETS_CONFIG[dataset_name]['loader'](folder)#, palette
...
class HyperX(torch.utils.data.Dataset):
    ...
    def __init__(self, data, gt,  **hyperparams):
        ...
        #print('\n\nxpos&ypos:', np.nonzero(mask))

        x_pos, y_pos = np.nonzero(mask)# if np.nonzero(mask)!=None else [], []
        """poss = []
        for i in x_pos:
            poss.append(i)"""
        ### change!!!!!! from //2 to /2
    ...
    def __getitem__(self, i):
        x, y = self.indices[i]
        ### change!!!!!!from //2 to /2
        x1, y1 = int(x - self.patch_size / 2), int(y - self.patch_size / 2)
        ...
        if self.center_pixel and self.patch_size > 1:
            ### change!!!!!! from //2 to /2
            label = label[int(self.patch_size / 2), int(self.patch_size / 2)]
        ...
        return data, label, self.indices[i]
